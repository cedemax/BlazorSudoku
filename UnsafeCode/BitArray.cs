...
namespace BlazorSudoku
{
    ...
    // shifting yourself.
    [Serializable]
    public sealed class CBitArray : ICollection, ICloneable
    {
        ...
        ** Exceptions: ArgumentOutOfRangeException if length < 0.
        =========================================================================*/
        public CBitArray(int length, bool defaultValue)
        {
            if (length < 0)
            {
        ...
        ** Exceptions: ArgumentException if bytes == null.
        ...
        public unsafe CBitArray(bool[] values)
        {
        ...
        ** Exceptions: ArgumentException if bits == null.
        =========================================================================*/
        public CBitArray(CBitArray bits)
        {
        ...
        **             value.Length != this.Length.
        =========================================================================*/
        public unsafe CBitArray And(CBitArray value)
        {
        ...
        /*=========================================================================
        ** Returns a reference to the current instance NANDed with value.
        **
        ** Exceptions: ArgumentException if value == null or
        **             value.Length != this.Length.
        =========================================================================*/
        public unsafe CBitArray NAnd(CBitArray value)
        {
            ArgumentNullException.ThrowIfNull(value);
            ...
            if (Vector256.IsHardwareAccelerated)
            {
                for (; i < (uint)count - (Vector256IntCount - 1u); i += Vector256IntCount)
                {
                    Vector256<int> result = Vector256.AndNot(Vector256.LoadUnsafe(ref left, i) , Vector256.LoadUnsafe(ref right, i));
                    result.StoreUnsafe(ref left, i);
                }
            }
            else if (Vector128.IsHardwareAccelerated)
            {
                for (; i < (uint)count - (Vector128IntCount - 1u); i += Vector128IntCount)
                {
                    Vector128<int> result = Vector128.AndNot(Vector128.LoadUnsafe(ref left, i) , Vector128.LoadUnsafe(ref right, i));
                    result.StoreUnsafe(ref left, i);
                }
            }
        ...
        **             value.Length != this.Length.
        =========================================================================*/
        public unsafe CBitArray Or(CBitArray value)
        {
            ArgumentNullException.ThrowIfNull(value);
        ...
        **             value.Length != this.Length.
        =========================================================================*/
        public unsafe CBitArray Xor(CBitArray value)
        {
            ArgumentNullException.ThrowIfNull(value);
        ...
        ** is updated and returned.
        ...
        public int PopCount()
        {
            int count0 = GetInt32ArrayLengthFromBitLength(m_length)-1;
            if (count0 < 0) // it was empty
                return 0;
            var pop = 0;

            for(var i = 0; i < count0; i++)
                pop += BitOperations.PopCount(unchecked((uint)m_array[i]));

            Div32Rem(m_length, out int extraBits);
            uint mask = uint.MaxValue >> (BitsPerInt32 - extraBits);
            pop += BitOperations.PopCount(unchecked((uint)m_array[^1]) & mask);
        ...
        ** Exceptions: ArgumentOutOfRangeException if count < 0
        =========================================================================*/
        public CBitArray RightShift(int count)
        {
            if (count <= 0)
            {
                if (count < 0)
        ...
        ** Exceptions: ArgumentOutOfRangeException if count < 0
        =========================================================================*/
        public CBitArray LeftShift(int count)
        {
            if (count <= 0)
            {
                if (count < 0)
        ...
        public object Clone() => new CBitArray(this);
        ...
        {
            private readonly CBitArray _bitArray;
            private int _index;
            private readonly int _version;
            private bool _currentElement;

            internal BitArrayEnumeratorSimple(CBitArray bitArray)
            {
                _bitArray = bitArray;
                _index = -1;
